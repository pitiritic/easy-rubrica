...
// --- 0. ACCIÓN: BORRAR / ANULAR EVALUACIÓN ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['borrar_evaluacion'])) {
    if ($currentUser['rol'] === 'admin' || $currentUser['rol'] === 'profesor') {
        $eval_id = (int)$_POST['eval_id'];
        $permitir_reintento = isset($_POST['permitir_reintento']) && $_POST['permitir_reintento'] == '1';
        try {
            if ($permitir_reintento) {
                $pdo->prepare("DELETE FROM evaluaciones WHERE id = ?")->execute([$eval_id]);
                Audit::log($pdo, "Evaluación Eliminada", "ID: $eval_id (Permite reintento)");
            } else {
                $pdo->prepare("UPDATE evaluaciones SET anulada = 1 WHERE id = ?")->execute([$eval_id]);
                Audit::log($pdo, "Evaluación Anulada", "ID: $eval_id (Bloquea reintento)");
            }
            $mensaje = "Acción procesada correctamente.";
        } catch (Exception $e) { $error = "Error al procesar la acción."; }
    }
}
...
if (isset($_GET['export_csv']) && $currentUser['rol'] !== 'alumno') {
    ...
    $sql = "SELECT e.fecha, cr.titulo as tarea, r.nombre as rubrica, c.nombre as clase,
            ...
            WHERE cr.autor_id = ? AND e.anulada = 0 AND " . ($tipo_export === 'tarea' ? "e.tarea_id = ?" : "e.evaluado_id = ?");
