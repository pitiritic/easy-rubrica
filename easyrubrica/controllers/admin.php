...
// A. CREAR TAREA (Con ponderaciones)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['crear_tarea'])) {
    try {
        ...
        $stmt->execute([
            trim($_POST['titulo']), (int)$_POST['clase_id'], (int)$_POST['rubrica_id'], 
            $currentUser['id'], (int)$_POST['peso_hetero'], (int)$_POST['peso_coeval'], (int)$_POST['peso_auto']
        ]);
        $mensaje = "Tarea creada con éxito.";
    } catch (Exception $e) { $error = "Error al crear: " . $e->getMessage(); }
}

// B. EDITAR TAREA (Desde el modal del diseño)
...
// C. CAMBIAR ESTADO (Abierta/Cerrada - Lógica de los candados)
...
$query_list = "SELECT cr.*, c.nombre as nombre_clase, r.nombre as nombre_rubrica 
