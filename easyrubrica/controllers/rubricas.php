...
if (isset($_GET['export_csv_id'])) {
    ...
    if ($rub) {
        $stmtC = $pdo->prepare("SELECT * FROM criterios WHERE rubrica_id = ?"); $stmtC->execute([$id_csv]); $criterios_csv = $stmtC->fetchAll();
        header('Content-Type: text/csv; charset=utf-8'); header('Content-Disposition: attachment; filename="export_rubrica_'.urlencode($rub['nombre']).'.csv"');
        ...
        foreach ($criterios_csv as $c) {
            $stmtN = $pdo->prepare("SELECT * FROM niveles WHERE criterio_id = ? ORDER BY valor ASC"); $stmtN->execute([$c['id']]); $niveles = $stmtN->fetchAll();
            $d = [1 => '', 2 => '', 3 => '', 4 => '']; foreach ($niveles as $n) { $d[$n['valor']] = $n['descriptor']; }
...
if (isset($_POST['guardar_rubrica'])) {
    $nombre = $_POST['nombre'] ?? ''; $asignatura = $_POST['asignatura'] ?? ''; $descripcion = $_POST['descripcion'] ?? '';
    $competencias_input = $_POST['competencias_hidden'] ?? ''; $competencias_array = array_filter(array_map('trim', explode(',', $competencias_input))); $competencias_json = json_encode(array_values($competencias_array));
    try {
        ...
        if (isset($_POST['id_rubrica']) && !empty($_POST['id_rubrica'])) {
            ...
            Audit::log($pdo, "Rúbrica Modificada", "Se actualizó la rúbrica: " . $nombre);
        } else {
            ...
            Audit::log($pdo, "Rúbrica Creada", "Nueva rúbrica generada: " . $nombre);
        }
        if (isset($_POST['criterio']) && is_array($_POST['criterio'])) {
            foreach ($_POST['criterio'] as $index => $nombre_criterio) {
                ...
                $stmtC = $pdo->prepare("INSERT INTO criterios (rubrica_id, nombre) VALUES (?, ?)"); $stmtC->execute([$id_rubrica, $nombre_criterio]);
                $id_criterio = $pdo->lastInsertId(); $etiquetas = [1 => 'Insuficiente', 2 => 'Aceptable', 3 => 'Bueno', 4 => 'Excelente'];
                for ($i = 1; $i <= 4; $i++) { $desc_nivel = $_POST["desc_{$index}_{$i}"] ?? ''; $pdo->prepare("INSERT INTO niveles (criterio_id, valor, etiqueta, descriptor) VALUES (?, ?, ?, ?)")->execute([$id_criterio, $i, $etiquetas[$i], $desc_nivel]); }
            }
        }
        $pdo->commit(); header("Location: ?action=rubricas&msg=" . urlencode("Operación exitosa")); exit;
...
if (isset($_POST['importar_csv'])) {
    if (isset($_FILES['archivo_csv']) && $_FILES['archivo_csv']['error'] === UPLOAD_ERR_OK) {
        $handle = fopen($_FILES['archivo_csv']['tmp_name'], "r"); fgetcsv($handle); 
        try {
            $pdo->beginTransaction(); $rub_id = null; $nom_ant = "";
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $cid = $pdo->lastInsertId(); $lbls = array(1=>'Insuficiente', 2=>'Aceptable', 3=>'Bueno', 4=>'Excelente');
            ...
            $pdo->commit(); 
            Audit::log($pdo, "Rúbrica Importada", "Se importó rúbrica mediante archivo CSV.");
...
if (isset($_POST['borrar_rubrica_id'])) {
    ...
    Audit::log($pdo, "Rúbrica Borrada", "Se eliminó la rúbrica ID: " . $_POST['borrar_rubrica_id']);
}
if (isset($_POST['duplicar_rubrica_id'])) {
    $id_org = (int)$_POST['duplicar_rubrica_id'];
    try {
        $pdo->beginTransaction();
        $stmt = $pdo->prepare("SELECT * FROM rubricas WHERE id = ?"); $stmt->execute([$id_org]); $r = $stmt->fetch();
        if ($r) {
            ...
            $nueva_id = $pdo->lastInsertId(); $stmtC = $pdo->prepare("SELECT * FROM criterios WHERE rubrica_id = ?"); $stmtC->execute([$id_org]); $criterios = $stmtC->fetchAll();
            foreach ($criterios as $crit) {
                $stmtInsCrit = $pdo->prepare("INSERT INTO criterios (rubrica_id, nombre) VALUES (?, ?)"); $stmtInsCrit->execute([$nueva_id, $crit['nombre']]); $nuevo_crit_id = $pdo->lastInsertId();
                $stmtN = $pdo->prepare("SELECT * FROM niveles WHERE criterio_id = ?"); $stmtN->execute([$crit['id']]); $niveles = $stmtN->fetchAll();
                foreach ($niveles as $niv) { $stmtInsNiv = $pdo->prepare("INSERT INTO niveles (criterio_id, valor, etiqueta, descriptor) VALUES (?, ?, ?, ?)"); $stmtInsNiv->execute([$nuevo_crit_id, $niv['valor'], $niv['etiqueta'], $niv['descriptor']]); }
            }
            Audit::log($pdo, "Rúbrica Duplicada", "Se duplicó la rúbrica ID: " . $id_org);
        }
