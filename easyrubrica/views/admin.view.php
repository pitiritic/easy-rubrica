                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th><a href="?action=usuarios&order_by=nombre&dir=<?= $dir==='ASC'?'desc':'asc' ?>" class="text-dark text-decoration-none">Nombre <i class="fa-solid fa-sort small ms-1"></i></a></th>
                            <th>Usuario</th>
                            <th>Rol</th>
                            <th>Clases</th>
...
<div class="modal fade" id="modalEditar" tabindex="-1">
    <div class="modal-dialog"><form method="POST" class="modal-content"><div class="modal-header bg-primary text-white"><h5>Editar Usuario</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><input type="hidden" name="editar_usuario" value="1"><input type="hidden" name="id" id="edit_id">
        ...
        <div class="mb-2"><label>Usuario</label><input type="text" name="usuario" id="edit_usuario" class="form-control" required></div>
        ...
        <div class="mb-2"><label>Rol</label><select name="rol" id="edit_rol" class="form-select"><option value="alumno">Alumno</option><?php if($currentUser['rol']==='admin'): ?><option value="profesor">Profesor</option><option value="admin">Admin</option><?php endif; ?></select></div>
        <div class="mb-2"><label>Password (opcional)</label><input type="password" name="password" class="form-control"></div>
        <div class="mb-2"><label>Clases</label><select name="clases[]" id="edit_clases" class="form-select" multiple><?php foreach($all_clases as $cl): ?><option value="<?= $cl['id'] ?>"><?= $cl['nombre'] ?></option><?php endforeach; ?></select></div>
